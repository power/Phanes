...
function secretsDump {
    ...
    for ($i = 1; $i -le 3; $i++)
    {
        ...
        $sid = (Get-ADUser $temp_user).Name
        C:\Users\Administrator\Desktop\Scripts\ntrights.exe +r SeAuditPrivilege -u FAKECOMPANY.LOCAL\$sid -m \\dc01
        C:\Users\Administrator\Desktop\Scripts\ntrights.exe +r SeAuditPrivilege -u FAKECOMPANY.LOCAL\$sid -m \\comp01
        $temp_accounts.RemoveAt($temp_accounts.IndexOf($temp_user))
        Write-Host "[+] $temp_user has privileges"
